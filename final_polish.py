...
for cell in nb['cells']:
    if cell['cell_type'] == 'markdown':
        ...
        # 12. OSD-21 is not in the signature
        ...
        # 19. & 21. Cached download fix
        if 'def download_study_file' in source:
            source = source.replace('logger.info(f"Downloading: {target_file}', 'if os.path.exists(local_path):\n            logger.info(f"Using cached file: {target_file}")\n        else:\n            logger.info(f"Downloading from NASA OSDR: {target_file}')
            
        # 11. OSD-21 mean aggregation wording
        ...
        if 'def map_mouse_to_human' in source:
            ...
            # Print mapping counts correctly
            ...
            # Add raw JSON export for provenance
            prov_export = '''
...
'''
        ...
        # 9. Cell 9 JSON Export (Since I moved export to Cell 9)
...
print("Final polished notebook modifications applied.")
