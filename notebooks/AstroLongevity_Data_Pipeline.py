...
for study_id in DATASETS:
    ...
    try:
        ...
        if study_id == "OSD-21":
            ...
            logger.info(f"OSD-21 cleanup: Applied probe-to-gene mean aggregation. {len(df)} unique genes retained.")
...
def map_mouse_to_human(mouse_symbols):
    ...
    # Maps mouse genes to human homologs using MyGene/HomoloGene. No uppercase fallbacks.
...
print(f"  {len(up_sig)} input upregulated mouse genes -> {len(up_genes_human)} human homologs")
